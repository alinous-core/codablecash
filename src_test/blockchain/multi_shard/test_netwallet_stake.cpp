...
TEST_GROUP(TestNetWalletStakeGroup) {
	TEST_SETUP(){
		env->setup();
	}
	TEST_TEARDOWN(){
...
TEST(TestNetWalletStakeGroup, case01){
...

	{
		...
		int count = inst->getMempoolTransctionCount();
		CHECK(count >= 30);
	...
	testnet.waitForBlockHeight(0, 0, 6);
