...
TEST_GROUP(TestJoinOrJoinGroup) {
	TEST_SETUP(){
		env->setup();
	}
	TEST_TEARDOWN(){
...
TEST(TestJoinOrJoinGroup, left01){
	const File* projectFolder = this->env->getProjectRoot();
	_ST(File, sourceFile, projectFolder->get(L"src_test/smartcontract_db/select/table_join/resources/orjoin/left01/select01.alns"))
	...
	/*	exobj = array->get(2);
		...
		UnicodeString ans2(L"{name: 'tanaka', id: 1, email: null, email_id: 11, email_id2: 111}");
		CHECK(str->equals(ans1));*/
	}
}

TEST(TestJoinOrJoinGroup, left02){ // left range index
	const File* projectFolder = this->env->getProjectRoot();
	_ST(File, sourceFile, projectFolder->get(L"src_test/smartcontract_db/select/table_join/resources/orjoin/left02/select01.alns"))
...
TEST(TestJoinOrJoinGroup, left03){ // left index
	const File* projectFolder = this->env->getProjectRoot();
	_ST(File, sourceFile, projectFolder->get(L"src_test/smartcontract_db/select/table_join/resources/orjoin/left03/select01.alns"))
